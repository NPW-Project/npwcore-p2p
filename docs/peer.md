# Peer
Represents a node from the p2p npw network. The Peer class supports connecting directly to other nodes or through a socks5 proxy like Tor.
...
// custom port
var livenetPeer = new Peer({host: '5.9.85.34', port: 61472});
var testnetPeer = new Peer({host: '5.9.85.34', port: 61474, network: Networks.testnet});
...
## Sending messages
In order to send messages the Peer class offers the `sendMessage(message)` method, which receives an instance of a message. All supported messages can be found in the `Messages` module.
